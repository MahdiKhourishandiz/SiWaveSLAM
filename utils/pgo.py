...
class PoseGraphManager:
    ...
    def add_loop_factor(
        ...
        if reject_outlier and not self.config.pgo_with_lm:
        ...
        try:
            with open(in_file, "r") as file:
                ...
                for i in range(0, len(lines), 5 * subsample_rate):
                    ...
                    indices = np.array(
                        [int(num) for num in lines[i].strip().split(" ")]
                    )
                    self.loop_edges.append(indices)

                    # Extract array data from the next 4 lines
                    array_data_lines = [
                        line.strip().split() for line in lines[i + 1: i + 4]
                    ]
                    tran_array = np.array(
                        [[float(num) for num in row] for row in array_data_lines]
                    )
    ...
    def get_pose_diff(self):
        assert self.pgo_poses.shape[0] == self.init_poses.shape[
            0], "poses before and after pgo must have the same size."
    ...
    def plot_loops(self, loop_plot_path, vis_now=False):
        ...
        for loop in self.loop_edges:
            ...
            node_1 = loop[1]
            ax.plot(
                [traj_est[node_0, 0], traj_est[node_1, 0]],
                [traj_est[node_0, 1], traj_est[node_1, 1]],
                [ts[node_0], ts[node_1]],
                color="green",
            )
