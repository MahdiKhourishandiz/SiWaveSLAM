...
CRIMSON = np.array([255/255, 49/255, 49/255])
OBSIDIAN = np.array([18/255, 18/255, 18/255])
AUREATE = np.array([162/255, 213/255, 242/255])
EMERALD = np.array([193/255, 247/255, 213/255])
AZURE = np.array([0/255, 255/255, 255/255])
...
class MapVisualizer:
    def __init__(self, config: Config = None):
        ...
        self.log_path = r"C:\Users\Mahdi\PycharmProjects\Siamese-Wavelet-SLAM\experiments\test_kitti_2025-04-24_16-41-48\log"
    ...
    def _initialize_visualizer(self):
        w_name = "Visualizer For SiWaveSLAM"
        ...
        if self.render_pointcloud:
            if scan is not None:
                ...
                if self.pc_uniform_color or (
                    ...
                    self.scan.paint_uniform_color(AUREATE)
            else:
    ...
    def _update_traj(
        ...
        if (self.render_trajectory and odom_poses_np is not None and odom_poses_np.shape[0] > 1):
            if pgo_poses_np is not None and (not self.render_odom_trajectory):
                ...
                if pgo_poses_np is None or self.before_pgo:
                    self.odom_traj.paint_uniform_color(CRIMSON)
                else:
                    self.odom_traj.paint_uniform_color(AZURE)
        ...
        if (
            ...
            self.pgo_traj.paint_uniform_color(CRIMSON)
            ...
            if self.render_pgo and loop_edges is not None and len(loop_edges) > 0:
                ...
                self.pgo_edges.paint_uniform_color(EMERALD)
        ...
        if (
            ...
            self.gt_traj.paint_uniform_color(OBSIDIAN)
            if odom_poses_np is None:
                self.gt_traj.paint_uniform_color(CRIMSON)
            if self.ego_view and cur_pose is not None:
